...
//    val inputpath = PathUtil.desktop("""key33.xlsx""")
//    val inputpath = PathUtil.desktop("""key44.xlsx""")
    val inputpath = PathUtil.desktop("""key55.xlsx""")
    ...
    var prevFactorNames:MutableList<String>? = null
    ...
    ExcelUtil.processRow(inputpath, 0, 2) { file, workbook, sheet, lastRowIdx, lastColIdx, rowIdx, row ->
        if (rowIdx >= 2) {
            ...
            //            println(cellValue)
            val keys = ExcelUtil.getCellValue(row, 1..4)
            ...
            if (StringUtils.isNotBlank(cellValue)) {
                ...
                if (cellValue.contains("-")) {
                    ...
                    keys.forEachIndexed { index, s ->
                        ...
                        val factorName = prevFactorNames!![index]

                        val generateName = "$factorName$min-$max"
...
//                        临时生成名字
//                        println("""INSERT INTO db_aries_questionnaire.DIM_RULE_FACTOR_KEYWORD(DIMENSION_ID,FACTOR_ID,MIN_SCORE,MAX_SCORE,KEYWORD) VALUES ('$prevDimId','$factorId','$min','$max','$generateName');""")
                    }
                } else {
                    //fac
                    ...
                    if (query.size == 1) {
                        ...
                        prevFactorNames = mutableListOf()
//                        println("$keys")

                        var relaSort = 1
                        keys.forEach {
                            val fac = jdbc.query("""SELECT ID FROM db_aries_questionnaire.DIM_FACTOR WHERE TITLE='$it' AND CREATE_TIME > '2019-02-17 00:00:00' """, ColumnMapRowMapper())
                            if (fac.size == 1) {
                                val FACTOR_ID = fac[0]["ID"].toString()
                                prevFactorIds!!.add(FACTOR_ID)
                                prevFactorNames!!.add(it)
