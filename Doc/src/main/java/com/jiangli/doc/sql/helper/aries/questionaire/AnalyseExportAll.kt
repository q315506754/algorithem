...
//        每个人各个维度得分
    ...
    println("过滤后用户总数：${ret.size} / ${userInfos.size}")

//    切分map
    val splitMapList =  mutableListOf<List<MutableMap<String,Any>>>()
    var mpOne = mutableListOf<MutableMap<String,Any>>()

    ret.forEachIndexed { index, mutableMap ->
        if (index % MAX_ROW_NUM == 0) {
            mpOne = mutableListOf<MutableMap<String,Any>>()
            splitMapList.add(mpOne)
        }

        mpOne.add(mutableMap)
    }

//    写入
    splitMapList.forEachIndexed { index, list ->
        println("写入excel ${index+1}/${splitMapList.size}")

        writeMapToExcel(PathUtil.desktop("行业数据all_${index+1}.xlsx"),list){
