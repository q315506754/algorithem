...
SELECT COUNT(DISTINCT(DIM_USER_ID)) AS C from db_aries_questionnaire.STATIS_USER_DIM sud
WHERE sud.USER_AGE>=${t.first} AND sud.USER_AGE <${t.last}
AND sud.JOB_CATEGORY=3
;
    """.trimIndent(),
                ColumnMapRowMapper())["C"]
...
SELECT t3.DIM_ID,dd.TITLE,COUNT(t3.DIM_ID) as MIN,
  COUNT(t3.DIM_ID) / (SELECT COUNT(DISTINCT(DIM_USER_ID)) FROM db_aries_questionnaire.STATIS_USER_DIM WHERE JOB_CATEGORY=3 )*100 as PERCENT
