 ...
 */
public class q239_sliding_window_dequeue {
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        ret[0] = nums[idxDeque.peekFirst()];


        for (int i = 1; i < nums.length - k + 1; i++) {
            int newIdx =  i + k - 1;
            while (!idxDeque.isEmpty() && nums[idxDeque.peekLast()] <= nums[newIdx]) {
                idxDeque.pollLast();
            }
            idxDeque.offerLast(newIdx);

            while (idxDeque.peekFirst() < i ) {
                idxDeque.pollFirst();
            }
            ret[i] = nums[idxDeque.peekFirst()];
        }
        return ret;
    }
}
